...
export function engineFixture(options: { transcriptImports?: import("../brain/import-admission.ts").TranscriptImportBinding[]; verifyPublicationNotSent?: import("../runtime/workflow-engine/engine.ts").WorkflowEngineOptions["verifyPublicationNotSent"]; store?: WorkflowExecutionStore; control?: StateStore; timerStore?: DurableTimerStore; artifact?: CompanyOSArtifact; recordsConnector?: Connector; batchConnector?: Connector; publicationConnector?: Connector; conversationForReceipt?: import("../runtime/workflow-engine/engine.ts").WorkflowEngineOptions["conversationForReceipt"] } = {}) {
  ...
  const engine = (pinned = artifact) => new WorkflowEngine({ transcriptImports: options.transcriptImports, verifyPublicationNotSent: options.verifyPublicationNotSent, artifact: pinned, store, control, timers, enabledWorkflowIds: artifact.workflows!.map((w) => w.id), operatorPrincipals: [ENGINE_OPERATOR],
