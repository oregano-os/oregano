...
test("explicit synthesis uses a qualified profile, validates membership, and falls back extractively on refusal", async () => {
  ...
  const profile: KnowledgeModelProfileBinding = { contractVersion: KNOWLEDGE_MODEL_EXECUTION_CONTRACT_VERSION, profile: "reasoning", profileVersion: "1", route: "test", model: "test/reasoning", secretRefs: ["secret:MODEL"], allowedDataClasses: ["restricted"], maxInputTokens: 10_000, maxOutputTokens: 2_000, maxCostUsd: 1, state: "active", qualification: { qualifiedAt: now, receiptId: "qualification:reasoning", adapterDigest: sha256("adapter") } };
  ...
  const refused = await synthesizeKnowledgeAnswer({ query: "Cedar launch", context, executor: { execute: async () => ({ output: {}, responseId: "refused", responseModel: "test/reasoning", inputTokens: 10, outputTokens: 0, costUsd: 0, latencyMs: 1, finishReason: "refusal" }) }, profile, authorizationContextDigest: sha256("auth"), dataClass: "restricted", grant: true, now });
