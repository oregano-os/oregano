...
test("ordinary Artifact compilation freezes generated scoped phases and rejects missing or oversized bindings", () => {
  ...
  try {
    ...
    const conversation = agentInstructions(agent, []);
    assert.ok(!conversation.includes("# Reviewed Brain phase:"), "Ordinary chat must not inline the entire import library");
    assert.ok(conversation.includes(agent.instructions));
    const { artifactHash, ...body } = artifact;
    assert.equal(artifactHash, sha256({ ...body, provenance: { ...body.provenance, builtAt: undefined } }), "The Artifact hash covers the exact phase-only material metadata");
