...
test("the additive records schema contains durable deduplication, projection, access, and reconciliation tables", () => {
  ...
  for (const table of ["source_events", "object_versions", "current_objects", "projection_rows", "access_decisions", "sync_receipts", "source_watermarks", "sync_leases", "durable_timers", "connector_echo_receipts"]) {
