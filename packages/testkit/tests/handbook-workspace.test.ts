...
const build = (workspaceRoot: string) => buildCompanyOSArtifact({
  workspaceRoot, instance, coreVersion: "0.16.1", coreCommit: "a".repeat(40),
...
test("plain Handbook articles need neither frontmatter nor a search index and remain scoped material", () => fixture((root) => {
  ...
  writeFileSync(pin, readFileSync(pin, "utf8").replaceAll("0.3.2", "0.16.1").replaceAll("0.1.0-experimental.7", "0.1.0-experimental.24"));
