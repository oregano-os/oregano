...
import { readCandidateReleaseEvidence, createPostgresReleasePrivateState, createPostgresReleaseRunStore } from "../../state-postgres/release-run-store.ts";
...
test("Postgres release snapshots retain exact acceptance, fence concurrent saves and recover expired leases", { skip: !enabled }, async () => {
  ...
  assert.equal((await readCandidateReleaseEvidence(id, candidate.id, now))[0]!.candidateDigest, run.candidateDigest);
  assert.deepEqual(await readCandidateReleaseEvidence("foreign", candidate.id, now), []);
  assert.deepEqual(await readCandidateReleaseEvidence(id, "foreign", now), []);
  assert.deepEqual(await readCandidateReleaseEvidence(id, candidate.id, "2031-04-05T11:00:00.000Z"), []);
