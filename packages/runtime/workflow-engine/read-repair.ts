...
export function validateReadRepairFeedback(value: unknown): asserts value is string {
  if (typeof value !== "string" || !value.trim() || value.length > 2000 || /[\u0000-\u001f\u007f]/.test(value)) throw new Error("Read repair feedback must be 1 to 2000 characters without control characters");
}

/** Feedback expires at the first repaired step; later phases never inherit it. */
export function workflowReadRepairFeedback(state: WorkflowMutableState) {
  const repair = state.readRepairs?.at(-1);
  if (state.status !== "running" || state.cursor !== repair?.fromStepId || repair.feedback === undefined) return undefined;
  validateReadRepairFeedback(repair.feedback);
  return { number: state.readRepairs!.length, feedback: repair.feedback, digest: sha256(repair.feedback) };
}
...
export function prepareWorkflowReadRepair(args: { artifact: CompanyOSArtifact; workflow: CompiledWorkflow; state: WorkflowMutableState; fromStepId: string; principal: string; now: string; reason: string; feedback?: string }): WorkflowMutableState {
  ...
  if (args.feedback !== undefined) validateReadRepairFeedback(args.feedback);
  ...
  const next = structuredClone(state), repair: WorkflowReadRepair = {
    ...
    reasonDigest: sha256(args.reason), ...(args.feedback === undefined ? {} : { feedback: args.feedback }), blocked: structuredClone(state.blocked),
...
export function validateWorkflowReadRepairs(state: WorkflowMutableState, workflow: CompiledWorkflow, artifact: CompanyOSArtifact, previous?: WorkflowMutableState): Set<string> {
  ...
  for (const [index, repair] of history.entries()) {
    if (!repair || Object.keys(repair).filter(key => key !== "feedback").sort().join(",") !== "authorizedAt,blocked,fromStepId,principal,reasonDigest,steps,throughStepId"
    ...
    if (Object.hasOwn(repair, "feedback")) validateReadRepairFeedback(repair.feedback);
  ...
  const expected = prepareWorkflowReadRepair({ artifact, workflow, state: previous, fromStepId: repair.fromStepId,
    principal: repair.principal, now: repair.authorizedAt, reason: "validated separately", ...(repair.feedback === undefined ? {} : { feedback: repair.feedback }) });
