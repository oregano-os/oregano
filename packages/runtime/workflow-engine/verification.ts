      ...
      for (const item of items) {
        ...
        if (step.decision) {
          ...
          input = renderWorkflowDecisionNotice({ runId: run.runId, workflowId: workflow!.id, stepId: step.id, role: decision.role,
            ...
            conversationReference: workflowDecisionThread(workflow!, step, itemContext, bindings[0]!.destinationBinding, "continueIn"), openThread: step.decision?.conversationRoot === true,
