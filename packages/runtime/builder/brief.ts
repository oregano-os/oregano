...
export const BUILDER_BRIEF_SCHEMA = {
  ...
  properties: {
    ...
    test: { type: "object", additionalProperties: false, required: ["strategy", "scenarios", "targetBindings"], properties: {
      ...
      execution: { oneOf: [
        { type: "object", additionalProperties: false, required: ["kind", "agentId", "prompt"], properties: { kind: { const: "agent" }, agentId: text, prompt: text, interaction: { type: "string", enum: ["automatic", "interactive"] } } },
...
export const BUILDER_INTAKE_INSTRUCTIONS = [
  ...
  "For a status question, screenshot evaluation or test-result question, read builder_read_test_result and answer the question. Do not silently revise, repeat an earlier proposal or start coding. Missing or unreadable images must be disclosed; use retained test evidence and do not pretend to see an attachment.",
  "Use builder_list_builds to locate an older request. An explicit new development request may start while previous builds remain open; never carry unrelated pending feedback into it. Revise an existing candidate only when the current human message requests that revision. A past build permission is consumed.",
  "An interactive test uses the user's selected build for new threads in the configured test channel; existing threads retain their version. The result card offers Go Live, Discard Build, Open Test Channel and Request Changes directly. No Finish test or Restart test action is needed. An idle test pauses after the configured period (seven days by default); resume the saved build without recoding when requested.",
  ...
  "Call the user's task a Build request (or your request), including new development; the resolved coding instructions are the Build brief. Use the company's conversation language. A queued acknowledgement does not prove that the coding agent started; only runtime progress may report that.",
  "Recommend the smallest supported test before coding. Explain in ordinary language what runs, where the human sees it, what data/actions it affects, what to inspect and its limits. Keep interaction, effects and hosting as internal reasoning; do not present a test menu or mandatory questionnaire. Reuse known defaults and let the human adjust your recommendation. Ask only about a material missing decision.",
  "Current tests are automatic technical checks (auto), a single Agent answer, an interactive Agent conversation without Tools, or a bounded operator workflow on designated test resources. Simulation, general Preview provisioning and bounded live trials are deferred; explain that they are unavailable instead of selecting simulate or live-trial. A Preview or second app is not needed for a supported interactive Agent test.",
  "Before selecting test-resources, call builder_instance_capabilities. Use only listed test resources and a supported concrete test.execution. Resolve missing capabilities before coding. An Agent test uses kind=agent, agentId, prompt and interaction=automatic or interactive. Recommend interactive when the human wants to try the Agent and ask follow-up questions; automatic produces only one answer for review. The Agent must have no Tools. Workflow tests use kind=workflow, workflowId and fields for an operator-opened graph without messages, timers or intermediate decisions. The report is a real post to the test destination; do not claim there are no Slack posts or that semantic correctness was automatically proven.",
  "Set deploymentIntent=after-acceptance when the human wants eventual publication only after their approval. This merely offers Go Live for the checked result and never grants acceptance. Use prepare-only only when the human explicitly wants a draft without a publication action. Refer to the final action as Go Live, not merge. Starting a build, completing a test and accepting publication are distinct.",
