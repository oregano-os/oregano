    ...
    const steps: CompiledWorkflowStep[] = rawSteps.map((raw, index) => {
      ...
      if (raw.tool === "agent") {
        ...
        return { ...base, kind: "agent", allowedTools: tools.map((entry: any) => entry.tool.runtimeId), maxRisk: maximumRisk(...tools.map((entry: any) => entry.tool.risk)),
          agent: { context: raw.context, instructions, ...(raw.failure_policy ? { failurePolicy: raw.failure_policy } : {}), ...(raw.instruction_selection !== undefined ? { instructionSelection: raw.instruction_selection } : {}), ...(raw.skills ? { skills: raw.skills } : {}), profile: raw.profile, task: raw.task, tools, ...(raw.completion === "text" ? { completion: "text" as const } : {}), outputSchema: raw.completion === "text" ? { type: "object", additionalProperties: false, required: ["text"], properties: { text: { type: "string", minLength: 1 } } } : raw.output_schema,
      ...
      if (raw.tool.startsWith("human:")) {
        ...
        return { ...base, kind: "decision", owner: raw.tool, tool: structuredClone(resolved), allowedTools: [resolved.runtimeId], maxRisk: resolved.risk, evidence: [...tool.contract.evidence], next: [...new Set([raw.approve, raw.reject, "end"])], decision: { ...(raw.continue_in ? { continueIn: raw.continue_in } : {}), ...(raw.thread ? { thread: raw.thread } : {}), ...(raw.recipient ? { recipient: raw.recipient } : {}), presentation, role: raw.tool.slice(6), binds: raw.binds, via: raw.via, timeoutBusinessDays: raw.timeout.business_days, calendarPath: path, targets: { approve: raw.approve, reject: raw.reject, timeout: "end" } } };
      }
