...
export async function recoverWorkflowReply(
  reference: { threadId: string; messageId: string },
  dependencies: {
    receive: (reference: { threadId: string; messageId: string }) => Promise<WorkflowInboundResult>;
  ...
  if (!identity || identity[1] !== conversation.accountId
    || reference.threadId !== `slack:${conversation.channelId}:${conversation.threadId}`
