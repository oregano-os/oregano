...
/** Optional shared-app guards. Excluded conversations belong to another destination. */
export async function ignoreSlackChannelEvent(request: Request, mode?: string, excludedChannelIds?: string, dmRecipients = workflowDmRecipients(), ownedChannelIds?: string): Promise<boolean> {
  ...
  if (!json && !form) return Boolean(owned);
  const body = await request.clone().text();
  // Ownership cannot be established outside the inspection limit; never fall through to a responder.
  if (body.length > 100_000) return Boolean(owned || channels.length || dmRecipients.length);
  try {
    ...
    if (payload?.type === "block_actions"
      && [payload?.channel?.id, payload?.container?.channel_id].some(channel => channels.includes(channel))) return true;
