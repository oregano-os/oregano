...
export async function ignoreSlackChannelEvent(request: Request, mode?: string, excludedChannelIds?: string, dmRecipients = workflowDmRecipients()): Promise<boolean> {
  ...
  if (mode !== "ignore" && !channels.length && !dmRecipients.length) return false;
