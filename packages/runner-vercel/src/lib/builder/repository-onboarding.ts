...
import { getGitHubRepositoryProvider } from "./provider-factory.ts";

export async function handleGitHubRepositoryOnboarding(
  request: Request,
  dependencies?: {
    readonly secret?: string;
    readonly provider?: Pick<GitHubAppRepositoryProvider, "verifyInstallation">;
  },
): Promise<Response> {
  const secret = dependencies?.secret ?? process.env.COMPANYOS_REPOSITORY_ONBOARDING_SECRET;
  if (!secret || request.headers.get("authorization") !== `Bearer ${secret}`) {
    return Response.json({ ok: false, error: "unauthorized" }, { status: 401 });
  }
  try {
    const body = await request.json() as Record<string, unknown>;
    const provider = dependencies?.provider ?? getGitHubRepositoryProvider();
