...
function fixture(prepared = candidate) {
  ...
  const integration = createBuilderReleaseIntegration({
    ...
    async prepareCandidate() { return prepared; }, fallback: { async deliver() { fallbacks++; } },
  });
  ...
  return { integration, click, messages, accepted, run, fallbacks: () => fallbacks };
}
