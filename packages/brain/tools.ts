...
export const BRAIN_READ_DEFINITIONS = [
  ...
  { name: "context_pack", description: "Budget-packed session-boundary bundle for selected entities: current page text, active attributed Takes and useful Timeline/source context, zero LLM calls. Call at session start (warm cold context) and after compaction (rehydrate what the summary lost). Packing is server-side; the response reports budget_used, omissions and unresolved/ambiguous names. Branch on structured fields, never prose. The starting change cursor and contents share one indexed snapshot; a changed topic list needs a new bundle.",
    inputSchema: object(["entities"], { entities: text("Comma-separated entity names/slugs to bundle. Capped at 8.", 2000), budget_tokens: { type: "integer", minimum: 1, maximum: 8000, description: "Server-side token budget (char/4). Current knowledge packs first, then active Takes and Timeline. Default 4000; maximum 8000." } }) },
