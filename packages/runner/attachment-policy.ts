...
export interface RepresentationLimits {
  readonly maxCount?: number;
  readonly maxTotalBytes?: number;
  readonly maxEncodedBytesPerFile?: number;
}
export interface AttachmentPolicy {
  ...
  readonly representationLimits?: Readonly<Partial<Record<"text" | "image" | "file", RepresentationLimits>>>;
  readonly modelOverrides?: readonly { readonly modelPatterns: readonly string[]; readonly representationLimits: AttachmentPolicy["representationLimits"] }[];
...
export function validateAttachmentPolicies(value: unknown): AttachmentPolicyConfiguration {
  ...
  for (const policy of Object.values(config.providers)) {
    ...
    for (const key of ["maxAttachments", "maxTotalBytes", "maxRequestBytes", "maxTextBytes"] as const) {
      if ((policy[key] !== undefined || key === "maxRequestBytes") && (!Number.isSafeInteger(policy[key]) || policy[key]! <= 0)) throw new Error(`Invalid attachment policy ${key}.`);
    }
    ...
    for (const [mime, format] of Object.entries(policy.formats)) {
      if (!/^[a-z]+\/[a-z0-9.+-]+$/.test(mime) || !["text", "image", "file"].includes(format.representation)
        || (format.maxBytes !== undefined && (!Number.isSafeInteger(format.maxBytes) || format.maxBytes <= 0))) throw new Error("Invalid attachment format policy.");
      Object.freeze(format);
    }
    for (const limits of [policy.representationLimits, ...(policy.modelOverrides ?? []).map(override => {
      if (!override.modelPatterns?.length || override.modelPatterns.some(pattern => typeof pattern !== "string" || !/^[a-zA-Z0-9/._-]+\*?$/.test(pattern))) throw new Error("Invalid attachment override model patterns.");
      Object.freeze(override.modelPatterns); Object.freeze(override);
      return override.representationLimits;
    })]) {
      if (!limits) continue;
      for (const [representation, limit] of Object.entries(limits)) {
        if (!["text", "image", "file"].includes(representation) || !limit) throw new Error("Invalid attachment representation limits.");
        for (const [key, value] of Object.entries(limit)) {
          if (!["maxCount", "maxTotalBytes", "maxEncodedBytesPerFile"].includes(key) || !Number.isSafeInteger(value) || value <= 0) throw new Error("Invalid attachment representation limit.");
        }
        Object.freeze(limit);
      }
      Object.freeze(limits);
    }
    Object.freeze(policy.modelOverrides);
...
export function attachmentPolicy(selection: { route: string; model: string }, configuration = CORE_ATTACHMENT_POLICIES): AttachmentPolicy {
  ...
  const override = policy.modelOverrides?.find(entry => entry.modelPatterns.some(pattern => pattern.endsWith("*")
    ? selection.model.startsWith(pattern.slice(0, -1)) : selection.model === pattern));
  return override ? { ...policy, representationLimits: { ...policy.representationLimits, ...override.representationLimits } } : policy;
}
