...
export function ensureCompanyOSSchema(): Promise<void> {
  migration ??= (async () => {
    ...
    await sql`create table if not exists companyos.builder_jobs (
      job_id text primary key, request_id text not null unique, fingerprint text not null,
      input jsonb not null,
      state text not null check (state in ('queued','preparing_source','executing','validating','publishing','published','failed','cancelled')),
      ...
      execution_handle jsonb, evidence jsonb, terminal_reason text,
    ...
    await sql`create table if not exists companyos.repository_installations (
      ...
      provider_repository_id text not null, repository_id text not null,
      owner_name text not null, repository_name text not null, default_branch text not null,
      status text not null check (status in ('active','suspended','revoked')),
      verified_at timestamptz not null, updated_at timestamptz not null,
