...
export const CORE_KNOWLEDGE_PROMPTS: readonly KnowledgePromptDefinition[] = [
  ...
  prompt({
    promptId: "knowledge.working-synthesis", version: "5", task: "working-synthesis", profile: "deep",
    inputSchemaId: "knowledge.working-synthesis.input@3", outputSchemaId: "knowledge.working-synthesis.output@2", fixtureSetId: "knowledge.working-synthesis.fixtures@1",
    systemInstruction: `${common} Produce versioned working memory, never official policy.`,
    userInstruction: "Synthesize only the supplied deterministic Claim segment chunkIndex of chunkCount for subjectIdentity. Return a concise working-synthesis body of at most 600 words so all exact Claim identity arrays fit inside the bounded structured result. Distinguish supported, contested, and superseded Claims; preserve meaningful minority positions and temporal changes. The three Claim identity arrays are mutually exclusive partitions: place any Claim in at most one array, choosing contested when support remains materially disputed and superseded only when later evidence explicitly replaces it. Claims that do not materially support the synthesis may be omitted. If validationFeedbackCode is present, regenerate the complete segment output and correct that validation failure. State uncertainty and gaps. Do not turn a proposal, prediction, or repeated assertion into a decision. The body must clearly identify itself as one segment of a working synthesis when readers could mistake it for policy. Segment outputs are merged deterministically after every segment succeeds.",
    evidenceRule: "List only the exact claimId JSON field values from supplied Claims in supportingClaimIds, contestedClaimIds, and supersededClaimIds; do not copy the outer evidenceId prefix. Never repeat an identity within or across those arrays, and never invent a Claim identity.",
  }),
  prompt({
