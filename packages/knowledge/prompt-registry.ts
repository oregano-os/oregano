...
export const CORE_KNOWLEDGE_PROMPTS: readonly KnowledgePromptDefinition[] = [
  ...
  prompt({
    promptId: "knowledge.claim-extraction", version: "3", task: "claim-extraction", profile: "reasoning",
    inputSchemaId: "knowledge.claim-extraction.input@1", outputSchemaId: "knowledge.claim-extraction.output@2", fixtureSetId: "knowledge.claim-extraction.fixtures@1",
    systemInstruction: `${common} Extract evidence-bound atomic Facts and attributed Takes into separate collections.`,
    userInstruction: "Create a concise Page title and neutral summary. Extract only durable, useful, atomic claims. A Fact records an evidenced event, preference, commitment, belief, or factual assertion owned by the accountable principal; a speaker or participant is not automatically that owner. A Take records an opinion, prediction, bet, hunch, or interpretive position and requires exactly one Holder. Classify an explicit forecast of a future outcome as a bet; use hunch only for a weak intuition without a stated forecast. Preserve exact supplied Holder identities. Preserve explicit source positions as source-literal Takes. Put novel model interpretation only in model-derived Takes with the system Holder and lower confidence; do not disguise it as a Fact. Do not extract greetings, repetition, conversational filler, or claims that cannot be located exactly. Split compound claims when their truth, owner, Holder, or time may differ. Calibrate extractionConfidence to extraction certainty and epistemicWeight to evidence strength. Include only explicitly evidenced participant relations. Use one-based original line ranges or exact timestamps.",
