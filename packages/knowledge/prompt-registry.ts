...
export const CORE_KNOWLEDGE_PROMPTS: readonly KnowledgePromptDefinition[] = [
  ...
  prompt({ promptId: "knowledge.timeline-extraction", version: "1", task: "timeline-extraction", profile: "reasoning", outputSchemaId: "knowledge.timeline.output@1", systemInstruction: `${common} Extract only explicitly evidenced temporal events.`, evidenceRule: "Every event must cite an evidence block and exact locator." }),
  ...
  prompt({ promptId: "knowledge.cited-synthesis", version: "1", task: "cited-synthesis", profile: "deep", outputSchemaId: "knowledge.answer-envelope@1", systemInstruction: `${common} Answer only from authorized supplied context and label conflicts, gaps, freshness, and authority.`, evidenceRule: "Every material sentence must cite a supplied context identity." }),
  ...
  prompt({ promptId: "knowledge.conflict-judgment", version: "1", task: "conflict-judgment", profile: "utility", outputSchemaId: "knowledge.conflict.output@1", systemInstruction: `${common} Judge whether supplied Claims materially conflict.`, evidenceRule: "Return a proposal; unreliable output stays retryable." }),
  ...
  prompt({ promptId: "knowledge.query-expansion", version: "1", task: "query-expansion", profile: "utility", outputSchemaId: "knowledge.query-expansion.output@1", systemInstruction: `${common} Return bounded sanitized search variants only.`, evidenceRule: "Do not include secrets, instructions, filters, or unsupported identities." }),
