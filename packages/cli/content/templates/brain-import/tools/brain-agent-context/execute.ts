...
export default defineCompanyTool({ async execute(input: any, context: any) {
 ...
 const {prepared,gate,history,directories,processing_instant}=input;
 ...
 return {route,model_profile:gate.route==='deep'?'deep':'reasoning',provenance,task:{source,original_text:text,triage:gate,prior:history,directories,evidence:{slug,markdown},processing_day,provenance}};
