...
export default defineCompanyTool({ async execute(input: any, context: any) {
  const { identity, version, default_projection, routes } = input;
  ...
  if (typeof version !== "string" || !version.length || version.length > 2000 || /[\x00-\x1f\x7f]/.test(version)) throw Error("Exact source version is required");
  const projection_id = routes.find((route: any) => identity.startsWith(route.identity_prefix))?.projection ?? default_projection;
  const records = await context.capabilities.call("records.query", { projection_id, filters: { identity, version }, all_pages: true });
  return { projection_id, records };
} });
