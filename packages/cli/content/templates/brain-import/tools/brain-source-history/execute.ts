...
export default defineCompanyTool({async execute(input:any,context:any){
 ...
 const history=await context.capabilities.call('evidence.query',{kind:'workflows',references:[input.workflow_id],from:input.history_from,to:input.cutoff,limit:100,
  match_fields:{source_identity:input.identity},include_feedback:false,step_ids:['finish-import','finish-one-shot','finish-discussion','finish-skip']}) as any;
 ...
 for(const run of history.items){
  ...
  const step=run.steps['finish-discussion']??run.steps['finish-one-shot']??run.steps['finish-import']??run.steps['finish-skip'],result=step?.output;
