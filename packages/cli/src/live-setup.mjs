...
export const resolveSlackApp = (executor, coreRoot, scope, connector, expectedTeamId, expectedProjectId) => {
  ...
  if (!Array.isArray(payload.data?.botScopes) || payload.data.botScopes.some((value) => typeof value !== "string")) throw new Error("The Slack connector did not report its bot permissions; setup cannot confirm that Oregano can post and read recent messages.");
  const missingBotScopes = VERCEL_NEON_SLACK_PROFILE.communication.requiredBotScopes.filter((value) => !payload.data.botScopes.includes(value));
  if (missingBotScopes.length) throw new Error(`The Slack app is missing bot permissions: ${missingBotScopes.join(", ")}. Add them under OAuth & Permissions → Bot Token Scopes at https://api.slack.com/apps/${encodeURIComponent(payload.data.appId)}/oauth, reinstall the app to the workspace, then continue setup.`);
