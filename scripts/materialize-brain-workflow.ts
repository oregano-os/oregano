...
meetings is an array of meeting summaries with slug, attendees and entities. verification contains exactly six objects, one each for check V1, V2, V3, V4, V5 and V6. Each status must be exactly "passed", "not-applicable" or "flagged-uncertainty"; never use "pass". These are proposed observations, not proof of saved-page verification. Put only a contiguous verbatim transcript span on a blockquote line under Notable Quotes, without ellipses, quotation marks, attribution or source notation on that same line; if no exact span exists, state that none was retained. Cite the source separately. gaps is an array of unresolved evidence gaps. No Markdown fence or explanatory prose outside the JSON object. If coverage cannot fit, return a bounded explicit gap rather than fabricated knowledge.
