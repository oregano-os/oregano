...
- The Builder release path now has maintained GitHub/Vercel adapters and default
  ...
  exact live health. Retained Records/Workflow bindings follow the new pairing; changed provider
  bindings or authorization policy need renewed Instance qualification. Lost provider create responses retain an intent rather than
...
- Core 0.14.0 and Workbench 0.1.0-experimental.22 retire Brain, Knowledge,
  ...
  use, without an active Knowledge consumer.
  Handbook content remains ordinary Workspace Markdown with the structured
...
- Deterministic Agent Bindings and `AgentResolver` select normal Company Agents,
  ...
  Agent Binding continues normal Agent operation. Its scheduled
...
- The experimental Workbench implements Guides, Change Plans, Core and
  ...
  builds. Its repository release candidate is `0.1.0-experimental.22`; no
...
2. Qualify the exact production migration and verify that obsolete Knowledge
   writers and dedicated bindings have been retired before deleting its schema.
3. Preserve and exercise the general model, Records and Agent capabilities.
